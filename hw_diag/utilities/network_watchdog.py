...
if os.access('/var/watchdog', os.W_OK):
    WATCHDOG_LOG_FILE_PATH = '/var/watchdog/watchdog.log'
    LAST_RESTART_FILE_PATH = '/var/watchdog/last_restart.json'
else:
    WATCHDOG_LOG_FILE_PATH = '/tmp/watchdog.log'
    LAST_RESTART_FILE_PATH = '/tmp/last_restart.json'
...
handler = RotatingFileHandler(WATCHDOG_LOG_FILE_PATH, maxBytes=MAX_LOG_SIZE, backupCount=3)
...
class NetworkWatchdog:
    ...
    def is_connected(self) -> bool:
        ...
        logger.info(f"Internet connectivity: {self.have_internet()}")
    ...
    def get_last_restart(self) -> datetime:
        try:
            last_restart = KeyStore(LAST_RESTART_FILE_PATH).get(LAST_RESTART_KEY)
            return datetime.strptime(last_restart, LAST_RESTART_DATE_FORMAT)
        except Exception as e:
            logger.info(f"Can not find the previous restart time: {e}")
            return datetime.min

    def save_last_restart(self) -> None:
        KeyStore(LAST_RESTART_FILE_PATH).set(LAST_RESTART_KEY,
